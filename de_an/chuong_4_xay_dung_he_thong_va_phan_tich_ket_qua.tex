...
\begin{itemize}
  ...
  \item \textbf{LiteLLM Proxy}: Lớp trừu tượng hóa nhà cung cấp LLM,
    cho phép cấu hình các mô hình của OpenAI (GPT-4o, GPT-4o-mini,
    GPT-5-mini, GPT-5.4-mini, \texttt{gpt-image-1}) hoặc các nhà cung
    cấp khác qua biến môi trường mà không thay đổi code agent.
\end{itemize}
...
\paragraph{Lớp nhận dạng và tri thức thương hiệu.}
\begin{itemize}
...
\paragraph{Lớp pipeline tạo nội dung.}
...
tin cậy cao, đề án xây dựng pipeline đánh giá ba lớp độc lập dựa
...
(GEval \tlcite{ref:liu2023} của DeepEval) đánh giá mâu thuẫn giữa đầu ra và retrieval
...
đều áp dụng QLoRA \tlcite{ref:dettmers2023} (chi tiết lý thuyết xem
Mục~\ref{subsec:sft}).
...
một chuyên gia sinh ra với sự hỗ trợ của AI và hiệu chỉnh thủ công, hướng
tới đa dạng về loại hình (đồ uống, ăn nhanh, ăn ngồi), phân khúc giá
(bình dân, trung cấp), và phong cách thương hiệu (năng động, truyền
thống, hiện đại). Mỗi mẫu bao gồm cặp (prompt với thông tin sản phẩm
mô phỏng tình huống thực, bài viết mẫu đã hiệu chỉnh), cho phép mô hình
học ánh xạ từ thông tin thô đến bài viết hoàn chỉnh theo phong cách
mong muốn. Quá trình hiệu chỉnh thủ công nhằm loại bỏ các bài có nội
dung quảng cáo thái quá, thiếu thông tin sản phẩm cụ thể, hoặc mang
văn phong AI rập khuôn dễ nhận ra.
...
0{,}597$, $-0{,}3\%$). Overall đạt $0{,}646$ --- vượt baseline 2B
($0{,}570$) nhưng còn cách đáng kể so với điểm tham chiếu thương mại
GPT-5.4-mini ($0{,}707$, $\Delta = -8{,}6\%$). Kết quả gợi ý rằng với
...
tiếp cận bổ sung: thay vì học từ nhãn kiểm duyệt, mô hình được tối ưu
theo một \emph{hàm thưởng rule-based} xây dựng từ các đặc trưng hình
thức có tương quan với tín hiệu tương tác (lượt thích, bình luận, chia
sẻ) trong tập bài đăng Facebook hiện có --- thay vì sử dụng trực tiếp
chỉ số tương tác làm phần thưởng. Phương pháp GRPO (Group Relative
Policy Optimization) \tlcite{ref:shao2024} được chọn vì (i)~không cần
huấn luyện reward
model riêng biệt, (ii)~không cần critic/value model như PPO
\tlcite{ref:schulman2017}, và
(iii)~tương thích với hàm reward rule-based, tiết kiệm tài nguyên phù
hợp với bối cảnh SME. Cơ sở lý thuyết GRPO đã được trình
...
\begin{table}[htbp]
  ...
  \begin{tabular}{p{0.32\textwidth}p{0.58\textwidth}}
    ...
    Quantization & QLoRA 16-bit (NF4, double quant,
                   bf16 compute) \\
    LoRA & $r=16$, $\alpha=32$, dropout $=0$;
           7 module (q/k/v/o proj,
           gate/up/down proj) \\
...
Kết quả này gợi ý rằng \emph{sự khớp miền} (domain alignment) giữa
dữ liệu huấn luyện GRPO và tác vụ mục tiêu có vai trò quan trọng, cần
được kiểm chứng thêm bằng các thực nghiệm RL khác trong nghiên cứu
tiếp theo. Hướng khắc phục đề xuất: sử dụng tập huấn luyện GRPO là
chính các bài viết marketing F\&B có số liệu tương tác thực tế, thay
vì tập review chéo miền.

Do phạm vi thực nghiệm RL trong đề án còn hạn chế (chỉ một cấu hình
GRPO với một tập dữ liệu chéo miền), các quan sát trên nên được hiểu
như những gợi ý thiết kế cho nghiên cứu tiếp theo hơn là kết luận chắc
chắn. Một số yếu tố có thể đáng lưu ý cho các thực nghiệm RL tương lai
trong bài toán sinh văn bản marketing gồm: (i)~tính khớp miền giữa
tập huấn luyện reward và tác vụ mục tiêu; (ii)~thiết kế hàm reward bao
quát hơn các tín hiệu hình thức đơn lẻ; (iii)~cân nhắc cường độ ràng
buộc KL theo khoảng cách miền. Các giả thuyết này là cơ sở cho hướng
phát triển GRPO được đề xuất tại Chương~\ref{chap:ket-luan}.
...
\begin{table}[htbp]
  ...
  \caption{So sánh mô hình nền, SFT và GRPO trên tập F\&B ($n=100$, judge: GPT-5.4 Azure); in đậm = giá trị tốt nhất trong nhóm mã nguồn mở.}
  \label{tab:final-comparison}
  \begin{tabular}{lcccccc}
    \toprule
    \textbf{Mô hình} & \textbf{Faith.} & \textbf{Expan.}
      & \textbf{Vibe} & \textbf{Overall} & \textbf{Var} & \textbf{Std} \\
    \midrule
    GPT-5.4-mini
...
\textbf{GRPO với dữ liệu chéo miền làm giảm chất lượng tổng thể.}
Overall $0{,}352$ của \texttt{qwen3.5:2b} + RL thấp hơn cả baseline
$0{,}570$ ($-38{,}2\%$). Kết quả này cho thấy hàm reward rule-based
được xây dựng từ tập Dining Review chưa thay thế được nhãn kiểm duyệt
trong bài toán tối ưu phong cách marketing. Tuy nhiên, đây là kết quả
có giá trị quan sát: nó gợi ý cần lưu ý \emph{domain alignment} khi
thiết kế hàm reward cho RL trong bài toán sinh văn bản marketing, và
là điểm cần kiểm chứng thêm trong các thực nghiệm RL tiếp theo.

Để xác định mô hình SFT tốt nhất có thực sự tiệm cận hiệu suất mô hình
thương mại hay không, chúng tôi thực hiện kiểm định thống kê cặp giữa
\texttt{qwen3.5:4b + SFT} và \texttt{GPT-5.4-mini} trên cùng $n = 100$
trường hợp. Kết quả được trình bày ở Bảng~\ref{tab:statistical_test}
và minh họa trực quan ở Hình~\ref{fig:model-comparison}.

\begin{table}[ht]
  \centering
  \small
  \caption{Kiểm định thống kê cặp GPT-5.4-mini vs \texttt{qwen3.5:4b + SFT} ($n=100$); * = $p < 0{,}05$; in đậm = khác biệt có ý nghĩa thống kê. Phân loại độ lớn hiệu ứng (Cohen's $d$): $|d|<0{,}2$ nhỏ; $0{,}2 \leq |d| < 0{,}5$ trung bình; $0{,}5 \leq |d| < 0{,}8$ lớn; $|d| \geq 0{,}8$ rất lớn.}
...
\begin{figure}[ht]
  ...
  \begin{tikzpicture}
    ...
    \end{axis}
  \end{tikzpicture}
  \caption{So sánh GPT-5.4-mini và \texttt{qwen3.5:4b + SFT} theo bốn chỉ số ($n=100$); error bars: CI 95\%; * = $p<0{,}05$ (paired $t$-test).}
  \label{fig:model-comparison}
\end{figure}

\textbf{Vibe: hai mô hình tương đương về mặt thống kê.} Mặc dù mean
của \texttt{qwen3.5:4b + SFT} ($0{,}648$) nhỉnh hơn không đáng kể so
với \texttt{GPT-5.4-mini} ($0{,}645$), cả paired $t$-test ($p =
0{,}753$) lẫn Wilcoxon ($p = 0{,}986$) đều xác nhận khoảng cách này
\textbf{không có ý nghĩa thống kê} (Cohen's $d = -0{,}03$). Đây là phát
hiện quan trọng nhất từ phân tích thống kê: SFT đã thành công định
hình phong cách Facebook-native F\&B tiếng Việt ngang bằng mô hình
thương mại.

\textbf{Expansion: khoảng cách thực sự và đáng kể.} Mức $0{,}777$ so
với $0{,}817$ ($\Delta = -0{,}040$) là khác biệt thật: $t(99) =
4{,}93$, $p < 0{,}001$, Cohen's $d = 0{,}49$ (mức trung bình). Đây là
chiều cần cải thiện tiếp theo --- thành phần đòi hỏi khả năng suy
luận và diễn giải lợi ích sản phẩm ở mức độ cao mà tinh chỉnh trên
500 mẫu chưa truyền tải đầy đủ.

\textbf{Faithfulness: khoảng cách nhỏ nhưng có ý nghĩa thống kê.}
Mức $0{,}637$ so với $0{,}659$ ($\Delta = -0{,}022$; $p = 0{,}021$;
$d = 0{,}23$, mức trung bình --- sát ngưỡng dưới) cho thấy mô hình
SFT vẫn còn đôi khi bỏ sót thực thể so với GPT, song mức chênh lệch
trung bình tuyệt đối chỉ $0{,}022$ điểm phản ánh khoảng cách thực tế
khá hạn chế.

\textbf{Overall: gap nhỏ, chủ yếu do Expansion kéo xuống.}
Điểm tổng thể đạt $0{,}687$ so với $0{,}707$ ($\Delta = -0{,}020$,
$p < 0{,}001$, $d = 0{,}37$, mức trung bình). Khoảng cách có ý nghĩa
thống kê và như phân tích từng chỉ số cho thấy, nó tập trung chủ yếu
ở Expansion (đóng góp khoảng $68\%$ vào khoảng cách Overall) --- chiều
có effect size lớn nhất ($d = 0{,}49$, sát ngưỡng trên của mức trung
bình); Vibe đã ngang bằng còn Faithfulness chỉ chênh lệch trung bình
$0{,}022$ điểm.

\textbf{\texttt{qwen3.5:4b} + SFT là lựa chọn tốt nhất trong nhóm
mã nguồn mở hiện tại}, với Overall $0{,}687$ so với $0{,}707$ của
GPT-5.4-mini ($\Delta = -0{,}020$, chỉ $-2{,}8\%$). Cộng thêm chi phí
vận hành thấp hơn đáng kể --- mô hình chạy cục bộ, không phụ thuộc API
thương mại theo lượt gọi --- đây là kết quả có ý nghĩa thực tiễn rõ
ràng cho SME ngành F\&B Việt Nam có ngân sách hạn chế.
...
suất mô hình thương mại lớn; đồng thời, kết quả không thuận lợi của
GRPO với dữ liệu chéo miền gợi ý vai trò quan trọng của
\emph{domain alignment} trong thiết kế hàm reward, một giả thuyết cần
được kiểm chứng thêm trong các thực nghiệm RL tiếp theo. Chương 5 sẽ
tổng kết đóng góp của đề tài, đánh giá hạn chế còn tồn tại và đề xuất
