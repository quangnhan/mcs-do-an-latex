...
\begin{table}[ht]
  \centering
  \small
  \caption{Kiểm định thống kê cặp GPT-5.4-mini vs \texttt{qwen3.5:4b + SFT} ($n=100$); * = $p < 0{,}05$; in đậm = khác biệt có ý nghĩa thống kê.}
...
0{,}753$) lẫn Wilcoxon ($p = 0{,}987$) đều xác nhận khoảng cách này
...
khoảng 2.000 mẫu chưa truyền tải đầy đủ.

\textbf{Faithfulness: khoảng cách nhỏ nhưng có ý nghĩa thống kê.}
Mức $0{,}637$ so với $0{,}659$ ($\Delta = -0{,}022$; $p = 0{,}021$;
$d = 0{,}23$, mức nhỏ) cho thấy mô hình SFT vẫn còn đôi khi bỏ sót
thực thể so với GPT, song effect size nhỏ phản ánh khoảng cách thực
tế khá hạn chế.

\textbf{Overall: gap nhỏ--trung bình, chủ yếu do Expansion kéo xuống.}
Điểm tổng thể đạt $0{,}687$ so với $0{,}707$ ($\Delta = -0{,}020$,
$p < 0{,}001$, $d = 0{,}37$). Khoảng cách có ý nghĩa thống kê nhưng
effect size ở ngưỡng nhỏ--trung bình, và như phân tích từng chỉ số
cho thấy, nó được phân bổ gần như hoàn toàn vào Expansion --- chiều
duy nhất có effect size trung bình; Vibe đã ngang bằng và Faithfulness
chỉ chênh lệch không đáng kể.
